...
test('the opportunity list loads with its columns', async ({ page }) => {
  await login(page);
  await page.goto('/oportunidades');
  await expect(page.getByRole('heading', { name: 'Oportunidades' })).toBeVisible();
  await expect(page.getByRole('columnheader', { name: 'Estágio' })).toBeVisible();
  await expect(page.getByRole('columnheader', { name: 'Valor estimado' })).toBeVisible();
});

test('a created opportunity appears on the operational list via search', async ({ page }) => {
  await login(page);
  const name = `E2E OppList ${Date.now()}`;
  ...
  // Contact + qualify, then create the opportunity from the qualified lead.
  ...
  await opportunity.locator('#optype').fill('Software corporativo');
  await opportunity.locator('#opvalue').fill('15000');
  await opportunity.getByRole('button', { name: 'Criar oportunidade' }).click();
  await expect(page.getByText('Oportunidade criada')).toBeVisible();

  // The new opportunity (title = the lead name) is on the operational list.
  await page.goto('/oportunidades');
  await page.locator('#q').fill(name);
